...
def calculate_the_ensemble_mean_not_participate_bc(path_cwrf_raw, raw_init_date, path_pbc):
    vnames = ['AGHT_PL','AQ2M','ASNOW','ASNOWH','ASWDNS','AT2M','ATSK','AU_PL','AV_PL','AXTSS','AXWICE','AXWLIQ','PSFC','RH','uv_10']
    for vname in vnames:
        ...
        datasets = [xr.open_dataset(file, chunks={'south_north': 23}) for file in files]
...
def calculate_the_ensemble_mean_participate_bc(path_pbc,raw_init_date):
    vnames            =  ['T2MAX','T2MIN','PRAVG','ASWDNS']
    for vname in vnames:
        pattern = f'{raw_init_date}_icbc01_exp*_{vname}_daily.nc'
        files = []
        for dirpath, dirnames, filenames in os.walk(path_pbc):
            for filename in filenames:
                if filename.startswith(pattern.split('*')[0]) and filename.endswith(pattern.split('*')[1]):
                    files.append(os.path.join(dirpath, filename))
        datasets = [xr.open_dataset(file, chunks={'south_north': 23}) for file in files]
...
def post_bias_correction(raw_init_date,var_name,user_server):
    ...
    if var_name == 'T2MAX':
        ...
        calculate_the_ensemble_mean_not_participate_bc(path_cwrf_raw,raw_init_date,path_pbc)
    ...
    for Eexp in Eexps:
        ...
        subset[var_name] = xr.where(adjusted_with_bottom_top ==np.nan, subset[var_name] , adjusted_with_bottom_top )
    ...
    if var_name == 'ASWDNS':
        ...
        calculate_the_ensemble_mean_participate_bc(path_pbc,raw_init_date)
