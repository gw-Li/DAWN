...
# python   both_anomaly_boosting_JJA.py  "T2MAX" 'squared_error'      ## CFS+CWRF
def main(arg1, arg2):
...
def process_point(i, j):
    ...
    C3D     = DATA_CWRF[:, :, :, i, j]
    X       = np.concatenate((C3D.reshape(-1, C3D.shape[2]).transpose(), DATA_CFS[:, :, i, j].transpose()), axis=1)
...
output_file = nc.Dataset(pathout+"Both_"+Vname+"_anomaly_JJA_"+lossfunc+".nc", "w", format="NETCDF4")
